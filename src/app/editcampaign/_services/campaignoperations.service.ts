...
export class CampaignOperationsService {
    ...
    getCampaignData(id, options) {
        return this.http.get(this._configService.getApiUrl() + 'api/campaign/edit/' + id, options)
            .map((response: Response) => {
                let res = response.json();
                return res;
            });
    ...
    updateCampaignStep1(data, options) {
        return this.http.post(this._configService.getApiUrl() + 'api/campaign/update', data, options)
            .map((response: Response) => {
                let res = response.json();
                return res;
            });
    ...
    updateCampaignStep2(data, options) {
        return this.http.post(this._configService.getApiUrl() + 'api/campaign/update', data, options)
            .map((response: Response) => {
                let res = response.json();
                return res;
    ...
    loadCampaignApplistData(cid) {
        let headers = new Headers();
        headers.append('Authorization', 'Bearer ' + JSON.parse(localStorage.getItem('_token')));
        let options = new RequestOptions({ headers: headers });

        return this.http.get(this._configService.getApiUrl() + 'api/campaign/get-apps-details?cid=' + cid, options)
            .map((response: Response) => {
                let res = response.json();
                return res;
            });
