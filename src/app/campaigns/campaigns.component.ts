...
@Component({
    ...
    templateUrl: './template/createcampaign.component.html',
...
export class CampaignsComponent implements OnInit, AfterViewInit {
    ...
    validateCampaignFirstStep: (MovingDirection) => boolean = (direction) => {
        ...
        /* if (ValidationHelper.isEmpty(this.campaignType) == true) {
             error += '<i class="fa fa-times"></i> Please select campaign type <br>';
             errorFlag = 1;
         } */
        ...
        /*  if (ValidationHelper.isNumber(this.campaignEcpx) == false) {
              error += '<i class="fa fa-times"></i> Please enter ecpx in numbers<br>';
              errorFlag = 1;
          } */
    ...
    validateCampaignSecondStep: (MovingDirection) => boolean = (direction) => {
        ...
        if (ValidationHelper.getArrayLength(this.campaignExchanges) == 0) {
            error += '<i class="fa fa-times"></i> Please attatch atleast one applist<br>';
            errorFlag = 1;
        }
    ...
    validateCampaignThirdStep() {
        ...
        for (var i = 0; i < this.campaignSelectedCreatives.length; i++) {
            selectedCreativeIds.push(this.campaignSelectedCreatives[i].id);
        }
        formData.append("creative_id", selectedCreativeIds);

        for (var i = 0; i < this.campaignCategories.length; i++) {
            categoriesOne.push(this.campaignCategories[i].id);
        }

        formData.append("category", categoriesOne);
        ...
        //formData.append("ecpm", this.campaignEcpx);
        ...
        formData.append("timezone", this.campaignTimeZone.id);
        ...
        formData.append("raw_day_parting_obj", JSON.stringify(this.dayPartingDataCountByRow));
        formData.append("status", this.campaignStatus);

        let headers = new Headers();
        headers.append('Authorization', 'Bearer ' + JSON.parse(localStorage.getItem('_token')));
        let options = new RequestOptions({ headers: headers });
        this._campaignOperations.uploadCampaign(formData, options).subscribe(
            data => {
                if (data.status == 1200) {
                    this.loading = false;
                    let cid = data.campaign_id;
                    let appObj1 = this.AppObj;

                    appObj1.forEach(function(o1) {
                        delete o1.appName;
                        delete o1.listName;
                        delete o1.exchangeId;
                        delete o1.exchange;
                    });

                    var bidArr: any = [{}];

                    bidArr[0].cid = data.campaign_id,
                        bidArr[0].bids = appObj1;
                        ...
                        data => {
                            if (data.status == 1200) {
                                ...
                                swal({
                                    position: 'center',
                                    type: 'success',
                                    title: 'Your campaign created successfully',
                                    showConfirmButton: false,
                                    timer: 3000,
                                    onClose: () => {
    ...
    checkIfCreativeExists(obj) {
        if (this.campaignSelectedCreatives.filter(e => e.id === obj.id).length > 0) {
            return true;
        } else {
            return false;
        }
    ...
    openBidUpdateOptions(content) {
        ...
        let applist = [];
        for (var i = 0; i < this.campaignExchanges.length; i++) {
            applist.push(this.campaignExchanges[i].id)
        }


        this._campaignOperations.loadCampaignApplistData(applist).subscribe(
            data => {
                if (data.status == 1200) {
                    this.loading = false;
                    let appObje = this.AppObj
                    let maxBid = this.campaignMaxBid
                    if (this.AppObj.length > 0) {
                        let data1 = data.data;
                        data1.forEach(function(o1) {
                            appObje.forEach(function(o2) {
                                if (o1.sid == o2.sid) {
                                    o1.dynamic_bid = o2.dynamic_bid;
                                }
                            });
                        });
                        ...
                        this.apps = data.data;
                        let maxBid = this.campaignMaxBid
                        this.apps.forEach(function(o1) {
                            if (o1.dynamic_bid == "-1") {
                                o1.dynamic_bid = maxBid
                            }
                        });
                    }
    ...
    _keyUp(event: any, app) {

    }


    saveApps() {
        this.AppObj = this.apps
        this.modalReference.close();
        this.toastr.success("Bid price modifications have been updated", 'Success');
    }

    closeAppModal() {
        swal({
            title: 'Are you sure?',
            text: "Please click 'Submit' to save your modifications",
            type: 'warning',
